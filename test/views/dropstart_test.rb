...
class DropstartTest < ActionView::TestCase
  DEFAULT  = 'Default Dropstart'.freeze
  CUSTOM   = 'Custom Dropstart'.freeze
  BINDINGS = 'Binding Dropstart'.freeze

  setup do
    render 'components/dropstarts', name: 'Todo', min: 1, max: 5

    @root      = document_root_element.at("//div[contains(@id, 'root')]")
    @default   = @root.at(".//div[contains(@id,  'default')]")
                      .at(".//*[contains(@class, 'dropstart')]")
  ...
  test "#{DEFAULT} should have the dropstart class" do
    assert(@default['class'].match?('dropstart'))
  end
